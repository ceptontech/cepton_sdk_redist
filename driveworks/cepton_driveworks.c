...
void cepton_image_callback(CeptonSensorHandle handle, size_t n_points,
  ...
  if (to_copy > 0) {
    memcpy(&point_buffer[buffer_size], c_points,
           sizeof(struct CeptonSensorImagePoint) * to_copy);
    buffer_size += to_copy;
  }
}
...
dwStatus _dwLidarDecoder_decodePacket(dwLidarDecodedPacket *output,
  ...
  LOG("_dwLidarDecoder_decodePacket(%c%c%c%c, %ld)\n", buffer[0], buffer[1],
      buffer[2], buffer[3], length);

  if (length == 0) length = last_validate_length;
  ...
  if (buffer_size > 0) {
    ...
    for (size_t i = 0; i < buffer_size; i++) {
      ...
      float hypotenuse_small =
          sqrt(square(p->image_x) + square(p->image_z) + 1.0f);
      ...
      // pOutSpherical[i].radius = sqrt(pOut[i].x*pOut[i].x +
      // pOut[i].y*pOut[i].y + pOut[i].z*pOut[i].z); pOutSpherical[i].theta =
      // acos(pOut[i].z/pOutSpherical[i].radius); pOutSpherical[i].intensity =
      // p->intensity;
    }
    buffer_size = 0;
  } else {
...
dwStatus _dwLidarDecoder_synchronize(const uint8_t *buffer, const size_t length,
                                     size_t *remaining) {
...
dwStatus _dwLidarDecoder_validatePacket(const uint8_t *buffer,
  ...
  LOG("_dwLidarDecoder_validatePacket(%c%c%c%c, length=%d)\n", buffer[0],
      buffer[1], buffer[2], buffer[3], (int)length);
  return DW_SUCCESS;
}
